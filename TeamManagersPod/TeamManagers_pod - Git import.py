...
if __name__ == "__main__":
    test_case = TestCase()
    df = pd.read_csv ('TeamManagers_pod - Git import.csv')
    #loop through each line in the csv file
    for index, row in df.iterrows():
        test_case = TestCase()
        test_case.set_test_case_title(row['Area'])
        test_case.add_test_case_steps(row['Title'])
        #test_case.add_test_case_expected_result(row['Expected Result'])
        #test_case.add_test_case_labels(row['Type (Negative / Positive)'])
        if row['Automated test'] == 'No':
        ...
        test_case.add_test_case_labels("Git Import")
