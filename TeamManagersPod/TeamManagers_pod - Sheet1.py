...
if __name__ == "__main__":
    test_case = TestCase()
    df = pd.read_csv ('TeamManagers_pod - Sheet1.csv')
    #loop through each line in the csv file
    for index, row in df.iterrows():
        test_case = TestCase()
        test_case.set_test_case_title(row['Area'] + ' - ' + row['Description'])
        test_case.add_test_case_steps(str(row['Steps']))
        test_case.add_test_case_expected_result(row['Expected Result'])
        #test_case.add_test_case_labels(row['Type (Negative / Positive)'])

        if row['Is Test Automated or Not'] == 'No':
            test_case.add_test_case_labels("Manual")
        if row['Is Test Automated or Not'] == 'Automated':
            test_case.add_test_case_labels("Automated")
        if row['Is Test Automated or Not'] == 'Cannot Automated':
            test_case.add_test_case_labels("Cannot Automated")
