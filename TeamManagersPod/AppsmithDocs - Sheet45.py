...
if __name__ == "__main__":
    print("Check with Raksha")
    exit(0)
    test_case = TestCase()
    df = pd.read_csv ('AppsmithDocs - Sheet45.csv')
    #loop through each line in the csv file
    for index, row in df.iterrows():
        test_case = TestCase()
        test_case.set_test_case_title(row['Admin page setting '])
        test_case.add_test_case_steps(row['Admin page setting '])
