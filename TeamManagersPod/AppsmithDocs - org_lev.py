...
if __name__ == "__main__":
    test_case = TestCase()
    df = pd.read_csv ('AppsmithDocs - org_lev.csv')
    #loop through each line in the csv file
    for index, row in df.iterrows():
        test_case = TestCase()
        test_case.set_test_case_title(row['use Cases for Leave organization'])
        test_case.add_test_case_steps(row['use Cases for Leave organization'])
        ...
        test_case.add_test_case_labels("Organization")
