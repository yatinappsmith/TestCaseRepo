...
if __name__ == "__main__":
    print("Need to work with Raksha")
    exit(0)
    test_case = TestCase()
    df = pd.read_csv ('AppsmithDocs - Generateapp.csv')
    #loop through each line in the csv file
    for index, row in df.iterrows():
        test_case = TestCase()
        test_case.set_test_case_title(row['Usecases'])
        test_case.add_test_case_steps(row['Usecases'])
