...
if __name__ == "__main__":
    test_case = TestCase()
    df = pd.read_csv ('AppsmithDocs - Welcomepage_localDevlopment.csv')
    #loop through each line in the csv file
    for index, row in df.iterrows():
        test_case = TestCase()
        test_case.set_test_case_title(row['Testcases for Welcome Page setup for local development'])
        test_case.add_test_case_steps(row['Testcases for Welcome Page setup for local development'])
        ...
        test_case.add_test_case_labels("Super Admin")
