...
if __name__ == "__main__":
    ...
    print("complete")
    test_case = TestCase()
    df = pd.read_csv ('NDX test cases - Clean URLs.csv')
    #loop through each line in the csv file
    for index, row in df.iterrows():
        test_case = TestCase()
        test_case.set_test_case_title(row['Feature'] + ' - ' + row['Steps'])
        test_case.add_test_case_steps(row['Steps'])
        test_case.add_test_case_expected_result(row['Expected'])
        #test_case.add_test_case_labels(row['TEST TYPE'])
        test_case.add_test_case_labels("Manual")
        test_case.add_test_case_labels("Clean URLs")
