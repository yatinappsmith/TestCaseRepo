...
if __name__ == "__main__":
    print("Complete")
    test_case = TestCase()
    df = pd.read_csv ('Integration test ideas - Twilio.csv')
    #loop through each line in the csv file
    for index, row in df.iterrows():
        test_case = TestCase()
        test_case.set_test_case_title(row['Feature']+' - '+row['EXPECTED RESULT'])
        test_case.add_test_case_steps(row['EXPECTED RESULT'])
        ...
        test_case.add_test_case_labels("Twilio")
